...
#' An expected OR can be specified and the support calculated for this relative to the
#' null value (which is assumed to be 1, but can also be specified). A likelihood function
#' is plotted for the obtained OR, with a likelihood interval added and expected OR
#' if specified.
...
if (!is.null(exp.OR)) { SexOR_null <- log(xah) - log(nullh) }

cat("\nSupport for observed OR ", orv, " (dashed line) versus null of ", null,
    " (green line) = ", round(S2way,3), "\n Support for expected OR (blue line,
    if specified) from null value = ", SexOR_null,
...
invisible(list(S.val = S2way, df = unname(lt$parameter), exp.OR = exp.OR,
