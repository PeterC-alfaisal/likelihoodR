...
#' An expected correlation can be specified and the support calculated for this relative to the
#' null value (which is assumed to be 0, but can also be specified). A likelihood function
...
#' m800 <- c(128.5,	126.1,	124.2,	132.5,	134.7,	132.5,	138.5,	127.9,	133.7,	132.2,
#' 136.1,	142.8,	125.8, 131.5,	137.1,	134.9,	146.7,	133.9,	146.4,	144.0,	133.4,
...
L_corr <- function(xv, yv, null=0, exp.r=NULL, L.int=2, alpha=.05) {
...
lines(c(null,null),c(0,exp(-((rtrans(r)-rtrans(null))/se)^2/2)), lty=1, col = "green") # add H prob as green line
if (!is.null(exp.r)) {
...
cat("\nSupport for observed correlation ", r, " (dashed line) \n versus null ", null,
    " (green line) = ", round(SM,3), sep= "",
    "\n Support for specified correlation of ", exp.r," (blue line) versus observed r = ", S1,
     "\n Relative support for specified correlation versus null = ", S_10,
    "\n S-", L.int," likelihood interval (red line) is: ",round(lolim,5), " to ", round(hilim,5),
    "\n\n P value = ", round(m$p.value,5), " \n N = ", N)
