...
L_2way_Factorial_ANOVA <- function(data, factor1, factor2, contrast1=NULL, contrast2=NULL) {
  ...
  cat("\nSupport for full model (including interaction) versus null = ", round(S_12c,3), sep= "",
      ...
      "\n\nFirst factor main effect F(", dfv[1],",", dfv[4],") = ", round(Fval[1],3), ", p = ", Pval[1],
      "\n Second factor main effect F(", dfv[2],",", dfv[4],") = ", round(Fval[2],3), ", p = ", Pval[2],
      "\n Interaction F(", dfv[3],",", dfv[4],") = ", round(Fval[3],3), ", p = ", Pval[3],
  ...
  invisible(list(S.12c = S_12c, S.12 = S_12, S.FMc = S_FMc, S.FM = S_FM,
                 ...
                 F.f1 = Fval[1], Pval.f1 = Pval[1], F.f2 = Fval[2], Pval.f2 = Pval[2],
                 F.int = Fval[3], Pval.int = Pval[3]))
}
