...
binpL <- function(a,n,alpha=0.05, toler=0.0001){
  ...
  cat("For probability of ", round(p,4), " (dashed line) \n Likelihood-based ", 100*(1-alpha),
      sep= "", "% confidence interval (red line) from ",
      c(round(xmin1$minimum,5), " to ", round(xmin2$minimum,5)))
  invisible(list(prob.val = p, succ.fail = c(a,r), alpha = alpha, conf.int = c(xmin1$minimum, xmin2$minimum),
