...
#' is provided. The t and p values for the test against the null value are given.
#' Uses Welch's test where homogeneity of variance is not required.
#'
#' @usage L_2S_ttest(data, group, veq=0, null=0, d=0.5, alt.2=NULL, L.int=2)
...
L_2S_ttest <- function(data, group, veq=0, null=0, d=0.5, alt.2=NULL, L.int=2) {
  ...
  cat("\nMaximum support for the observed mean ", m.obs, " (dashed line) against the null ",
      ...
      "\n\n S-", L.int, " likelihood interval (red line) is from ", c(round(lolim,5),
                                                                      " to ", round(hilim,5)),
      "\n\nt(", round(df,1), ") = ", round(tval,3), ", p = ", pval, "\n ")
