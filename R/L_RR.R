...
#' An expected RR can be specified and the support calculated for this relative to the
#' null value (which is assumed to be 1, but can also be specified). A likelihood function
#' is plotted for the obtained RR, with a likelihood interval added and expected RR
#' if specified.
...
#'
#' @usage L_RR(table, null=1, exp.RR=NULL, L.int=2, alpha=0.05, cc=FALSE, toler=0.0001)
...
#' $exp.OR - expected RR.
#'
#' $S.exp.ORvsNull - support for expected RR versus the null.
#'
...
L_RR <- function(table, null=1, exp.RR=NULL, L.int=2, alpha=0.05, cc=FALSE, toler=0.0001) {
  ...
  if (!is.null(exp.RR)) { SexRR_null <- log(xah) - log(nullh) }

  cat("\nSupport for obseved RR ", rr, " (dashed line) versus null of ", null,
      " (green line) = ", round(S2way,3), "\n Support for expected RR (blue line,
      if specified) from null value = ", SexRR_null, sep= "", "\n   S-", L.int,
