...
#' If the contrasts are not specified then it calculates a linear versus a
...
#' two contrasts support since they both involve 1 parameter. Unequal group sizes can be used.
#'
#' @usage L_1way_ANOVA(data, group, cont1=NULL, cont2=NULL)
#'
...
#' @param cont1 first contrast, default = NULL.
#' @param cont2 second contrast, default = NULL.
#'
#' @return
...
L_1way_ANOVA <- function(data, group, cont1=NULL, cont2=NULL) {
...
SS_1 <- sum(cont1*gp_means)^2/(sum(cont1^2/(gp_n)))
SS_2 <- sum(cont2*gp_means)^2/(sum(cont2^2/(gp_n)))
...
cat("Support for group means versus null = ", round(S_12c,3), sep= "",
    "\n Support for contrast 1 versus contrast 2 = ", S_cont_12)
